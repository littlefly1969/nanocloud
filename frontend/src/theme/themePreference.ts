...
//
// Storage is LOCAL ONLY (no backend field, no migration). The key holds one of
// exactly three bounded tokens; anything else — a stale value, a value written
// by a future version, a hostile value — reads back as "absent", so the default
// applies instead of leaking an unknown string into the DOM.

// Versioned key: a future incompatible preference shape gets `.v2`, so old
// values are ignored rather than misinterpreted.
export const THEME_STORAGE_KEY = 'nanocloud.theme';
...
export function readStoredThemePreference(): ThemePreference {
  ...
  try {
    return toThemePreference(window.localStorage.getItem(THEME_STORAGE_KEY)) ?? DEFAULT_THEME_PREFERENCE;
  } catch {
