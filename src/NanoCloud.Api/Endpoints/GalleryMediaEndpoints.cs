...
public static class GalleryMediaEndpoints
{
    ...
    public static IEndpointRouteBuilder MapGalleryMediaEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapGet("/api/images", async (
            ...
            [FromServices] NanoCloud.Api.Ai.Photos.PhotoSimilarityService similarity,
            [FromServices] NanoCloud.Api.Ai.Photos.GallerySemanticQueryService semantic,
            CancellationToken cancellationToken) =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(semanticQuery))
            {
                ...
                NanoCloud.Api.Ai.Photos.GallerySemanticPage semanticPage;
                try
                {
                    semanticPage = await semantic.SearchAsync(
                        ownerUserId, effectiveLimit, cursor, semanticFilters, cancellationToken);
                }
                catch (NanoCloud.Api.Ai.Photos.SemanticSearchCursorException)
                {
        ...
        app.MapPost("/api/images/interpret-command", async (
            [FromBody] NanoCloud.Api.Ai.NaturalGallery.InterpretCommandRequest request,
            HttpContext httpContext,
            [FromServices] NanoCloud.Api.Ai.NaturalGallery.NaturalGalleryCommandService interpreter,
            CancellationToken cancellationToken) =>
        {
            ...
            {
                NanoCloud.Api.Ai.NaturalGallery.InterpretOutcomeKind.Ok => Results.Ok(outcome.Response),
                NanoCloud.Api.Ai.NaturalGallery.InterpretOutcomeKind.Unsupported =>
                    Results.Json(new { error = "unsupported_command" }, statusCode: StatusCodes.Status422UnprocessableEntity),
                NanoCloud.Api.Ai.NaturalGallery.InterpretOutcomeKind.ModelBusy =>
                    Results.Json(new { error = "model_busy" }, statusCode: StatusCodes.Status429TooManyRequests),
                NanoCloud.Api.Ai.NaturalGallery.InterpretOutcomeKind.Timeout =>
                    Results.Json(new { error = "model_timeout" }, statusCode: StatusCodes.Status504GatewayTimeout),
                NanoCloud.Api.Ai.NaturalGallery.InterpretOutcomeKind.ModelUnavailable =>
        ...
        static IResult MapMediaCollectionResult(
            NanoCloud.Api.Media.MediaCollectionResult result, int limit)
        {
            switch (result.Status)
            {
                case NanoCloud.Api.Media.MediaCollectionStatus.Ok:
                ...
                case NanoCloud.Api.Media.MediaCollectionStatus.AlbumNotFound:
                    return Results.NotFound();
                default:
        ...
        app.MapGet("/api/media", async (
            ...
            [FromQuery] bool? hasAudio,
            HttpContext httpContext,
            [FromServices] NanoCloud.Api.Media.IMediaCollectionQueryService media,
            CancellationToken cancellationToken) =>
        {
            var ownerUserId = httpContext.GetCurrentUserId()!.Value;
            if (!NanoCloud.Api.Media.MediaCollectionQueryBinder.TryBind(
                new NanoCloud.Api.Media.MediaCollectionSource.Library(),
            ...
            CancellationToken cancellationToken) =>
        {
            var query = q?.Trim() ?? string.Empty;
            if (query.Length == 0
                || query.Length > NanoCloud.Api.Media.Semantic.MediaSemanticSearchService.MaxQueryLength)
            {
                return Results.BadRequest(new
                {
                    error = $"'q' must contain 1-{NanoCloud.Api.Media.Semantic.MediaSemanticSearchService.MaxQueryLength} characters.",
                });
            }
            ...
            return Results.Ok(new NanoCloud.Api.Media.Semantic.SemanticMediaSearchResponse(
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            if (!NanoCloud.Api.Media.MediaCollectionQueryBinder.TryBind(
                new NanoCloud.Api.Media.MediaCollectionSource.Album(albumId),
                limit, cursor, scope, kind, q, favorite, minRating, dateTakenFrom, dateTakenTo,
