...
public static class PartyEndpoints
{
    ...
    public static IEndpointRouteBuilder MapPartyEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapGet("/api/party/{token}", async (
            ...
            [FromServices] NanoCloud.Api.Party.IPartyLinkService party,
            [FromServices] NanoCloud.Api.Party.IPartyMediaService partyMedia,
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            return Results.Ok(new NanoCloud.Api.Party.PartyAlbumDto(header.Name, header.ItemCount));
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            {
                var isVideo = i.Kind == NanoCloud.Api.Party.PartyMediaKind.Video;
                return new NanoCloud.Api.Party.PartyItemDto(
                    i.FileItemId,
                    isVideo ? "video" : "image",
            ...
            return Results.Ok(new NanoCloud.Api.Party.PartyItemsDto(header.Name, dtos));
        ...
        app.MapGet("/api/party/{token}/media/{fileId:guid}/thumbnail", (
            ...
            [FromServices] NanoCloud.Api.Metadata.IImageMetadataStripper stripper,
            CancellationToken cancellationToken) =>
        ...
        app.MapPost("/api/party/{token}/upload", async (
            ...
            HttpContext httpContext,
            [FromServices] NanoCloud.Api.Party.IPartyLinkService party,
            [FromServices] NanoCloud.Api.Party.IPartyUploadService uploads,
            [FromServices] IAuditLogger audit,
            CancellationToken cancellationToken) =>
        {
            SetNoStore(httpContext);
            ...
            {
                NanoCloud.Api.Party.PartyUploadOutcome outcome;
                try
                {
                ...
                {
                    outcome = NanoCloud.Api.Party.PartyUploadOutcome.Failed;
                }

                if (outcome == NanoCloud.Api.Party.PartyUploadOutcome.Accepted) accepted++;
                else rejected++;
            }
            ...
            return Results.Ok(new NanoCloud.Api.Party.PartyUploadResultDto(accepted, rejected));
        ...
        app.MapPost("/api/party/{token}/face-search", async (
            ...
            HttpContext httpContext,
            [FromServices] NanoCloud.Api.Party.IPartyLinkService party,
            [FromServices] NanoCloud.Api.Party.IPartyFaceSearchService faceSearch,
            [FromServices] IAuditLogger audit,
            CancellationToken cancellationToken) =>
        {
            SetNoStore(httpContext);
            ...
            var dto = new NanoCloud.Api.Party.PartyFaceSearchResponseDto(
                outcome.Status, outcome.SearchId, outcome.ResultCount,
                PartyImageItems(token, outcome.FileItemIds));

            return outcome.Status switch
            {
                NanoCloud.Api.Domain.PartyFaceSearchStatuses.Unavailable =>
                    Results.Json(dto, statusCode: StatusCodes.Status503ServiceUnavailable),
                NanoCloud.Api.Domain.PartyFaceSearchStatuses.InvalidImage =>
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            var dto = new NanoCloud.Api.Party.PartyFaceSearchResponseDto(
                NanoCloud.Api.Domain.PartyFaceSearchStatuses.Ready,
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            {
                NanoCloud.Api.Party.PartyFaceSearchActivationStatus.Activated =>
                    Results.Ok(new NanoCloud.Api.Party.PartyFaceSearchActivationDto(
                        searchId, result.ActivationVersion!.Value)),
                NanoCloud.Api.Party.PartyFaceSearchActivationStatus.NoMatches =>
                    Results.Json(new { error = "no_matches" }, statusCode: StatusCodes.Status409Conflict),
                NanoCloud.Api.Party.PartyFaceSearchActivationStatus.StaleSearch =>
        ...
        app.MapGet("/api/albums/{albumId:guid}/party-uploads", async (
            Guid albumId,
            HttpContext httpContext,
            [FromServices] NanoCloud.Api.Party.IPartyModerationService moderation,
            CancellationToken cancellationToken) =>
        {
            var ownerUserId = httpContext.GetCurrentUserId()!.Value;
        ...
        app.MapPost("/api/albums/{albumId:guid}/party-uploads/{fileItemId:guid}/hide", async (
            ...
            await ModeratePartyUploadAsync(
                ...
                NanoCloud.Api.Domain.PartyUploadStatuses.Hidden, AuditActions.PartyUploadHide, cancellationToken))
        ...
        app.MapPost("/api/albums/{albumId:guid}/party-uploads/{fileItemId:guid}/approve", async (
            ...
            await ModeratePartyUploadAsync(
                httpContext, moderation, audit, albumId, fileItemId,
                NanoCloud.Api.Domain.PartyUploadStatuses.Approved, AuditActions.PartyUploadApprove, cancellationToken))
        ...
        app.MapPost("/api/albums/{albumId:guid}/party-uploads/{fileItemId:guid}/reject", async (
            ...
            await ModeratePartyUploadAsync(
                httpContext, moderation, audit, albumId, fileItemId,
                NanoCloud.Api.Domain.PartyUploadStatuses.Rejected, AuditActions.PartyUploadReject, cancellationToken))
        ...
        app.MapPost("/api/albums/{albumId:guid}/party-uploads/{fileItemId:guid}/restore", async (
            ...
            await ModeratePartyUploadAsync(
                httpContext, moderation, audit, albumId, fileItemId,
                NanoCloud.Api.Domain.PartyUploadStatuses.Approved, AuditActions.PartyUploadRestore, cancellationToken))
    ...
    private static async Task<IResult> ServePartyMediaAsync(
        ...
        string variant,
        HttpContext httpContext,
        NanoCloud.Api.Party.IPartyLinkService party,
        NanoCloud.Api.Party.IPartyMediaService partyMedia,
        IFileThumbnailService thumbnails,
        NanoCloud.Api.Metadata.IImageMetadataStripper stripper,
        CancellationToken cancellationToken)
    {
        var access = await party.ResolvePublicAsync(token, cancellationToken);
        if (access is null)
        ...
        if (kind == NanoCloud.Api.Party.PartyMediaKind.Video && variant == "download")
        {
            return Results.NotFound();
        }

        string size = kind == NanoCloud.Api.Party.PartyMediaKind.Video
    ...
    private static IReadOnlyList<NanoCloud.Api.Party.PartyItemDto> PartyImageItems(
        string token, IReadOnlyList<Guid> ids)
    {
        var enc = Uri.EscapeDataString(token);
        return ids.Select(id => new NanoCloud.Api.Party.PartyItemDto(
            id,
            "image",
    ...
    private static async Task<IResult> ModeratePartyUploadAsync(
        HttpContext httpContext,
        NanoCloud.Api.Party.IPartyModerationService moderation,
        IAuditLogger audit,
        Guid albumId,
        Guid fileItemId,
        string status,
