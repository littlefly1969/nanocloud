...
public static class AdminJobsEndpoints
{
    public static IEndpointRouteBuilder MapAdminJobsEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapGet("/api/admin/jobs/catalog", async (
            [FromServices] NanoCloud.Api.Admin.AdminJobCatalogService catalog,
            CancellationToken cancellationToken) =>
        ...
        app.MapPost("/api/admin/jobs/enqueue", async (
            [FromBody] NanoCloud.Api.Admin.AdminJobEnqueueRequest? body,
            HttpContext httpContext,
            [FromServices] NanoCloud.Api.Jobs.IJobQueue jobs,
            [FromServices] NanoCloud.Api.Admin.AdminJobCatalogService catalog,
            [FromServices] NanoCloud.Api.Data.AppDbContext db,
            [FromServices] IAuditLogger audit,
            CancellationToken cancellationToken) =>
        {
            var command = NanoCloud.Api.Admin.AdminJobCommands.Find(body?.Command);
            if (command is null)
            {
                return Results.BadRequest(new { error = "Unknown job command." });
            }

            var values = NanoCloud.Api.Admin.AdminJobCommandBinder.Bind(command, body?.Params, out var bindError);
            if (values is null)
            {
            ...
            return Results.Ok(new NanoCloud.Api.Admin.AdminJobEnqueueResponse(
