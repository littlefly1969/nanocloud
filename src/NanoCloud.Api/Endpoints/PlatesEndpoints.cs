...
public static class PlatesEndpoints
{
    public static IEndpointRouteBuilder MapPlatesEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapGet("/api/plates/images/{id:guid}/thumbnail", async (
            ...
            HttpContext httpContext,
            [FromServices] IPlateImageService plates,
            [FromServices] NanoCloud.Api.Plates.Redaction.IPlateRedactedMediaService redacted,
            CancellationToken cancellationToken) =>
        {
            var ownerUserId = httpContext.GetCurrentUserId()!.Value;
            if (blurFaces == true)
            {
                return await ServeRedactedPlateMediaAsync(
                    ...
                    NanoCloud.Api.Plates.Redaction.PlateRedactionSourceKind.Thumbnail, cancellationToken);
            }
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            if (blurFaces == true)
            {
                return await ServeRedactedPlateMediaAsync(
                    redacted, httpContext, ownerUserId, id,
                    NanoCloud.Api.Plates.Redaction.PlateRedactionSourceKind.Preview, cancellationToken);
            }
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            if (blurFaces == true)
            {
                var result = await ServeRedactedPlateMediaAsync(
                    redacted, httpContext, ownerUserId, id,
                    NanoCloud.Api.Plates.Redaction.PlateRedactionSourceKind.Original, cancellationToken);
    ...
    private static async Task<IResult> ServeRedactedPlateMediaAsync(
        NanoCloud.Api.Plates.Redaction.IPlateRedactedMediaService redacted,
        HttpContext httpContext,
        Guid ownerUserId,
        Guid id,
        NanoCloud.Api.Plates.Redaction.PlateRedactionSourceKind kind,
        CancellationToken cancellationToken)
    {
        try
        {
        ...
        {
            return Results.Json(
                new { error = NanoCloud.Api.Plates.Redaction.PlateFaceRedactionUnavailableException.Code },
                statusCode: StatusCodes.Status409Conflict);
        }
        catch (NanoCloud.Api.Plates.Redaction.PlateRedactionImageTooLargeException)
        {
            return Results.Json(
                new { error = NanoCloud.Api.Plates.Redaction.PlateRedactionImageTooLargeException.Code },
                statusCode: StatusCodes.Status413PayloadTooLarge);
        }
    }
