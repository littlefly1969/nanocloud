...
using NanoCloud.Api.Ai.Faces;
using NanoCloud.Api.Ai.Jobs;
using NanoCloud.Api.Ai.Onnx;
using NanoCloud.Api.Ai.Onnx.Face;
...
using NanoCloud.Api.Folders;
using NanoCloud.Api.Audit;
using NanoCloud.Api.Jobs;
using NanoCloud.Api.Jobs.Handlers;
using NanoCloud.Api.Plates;
using NanoCloud.Api.MediaLibrary;
using NanoCloud.Api.Organizer;
using NanoCloud.Api.PhotoExport;
using NanoCloud.Api.Metadata;
using NanoCloud.Api.Storage;
using NanoCloud.Api.Uploads;
using NanoCloud.Api.Users;
...
public static class CliEntryPoint
{
        ...
        TextWriter stderr)
    {
        var email = ReadOption(args, "--email")
            ?? ReadEnv("NANO_CLOUD_ADMIN_EMAIL");
        var displayName = ReadOption(args, "--display-name")
            ?? ReadEnv("NANO_CLOUD_ADMIN_DISPLAY_NAME");
        var password = ReadOption(args, "--password")
            ?? ReadEnv("NANO_CLOUD_ADMIN_PASSWORD");
        var updatePassword = HasFlag(args, "--update-password")
            || string.Equals(ReadEnv("NANO_CLOUD_ADMIN_UPDATE_PASSWORD"), "true",
                StringComparison.OrdinalIgnoreCase);
        var makeAdmin = HasFlag(args, "--admin")
            || string.Equals(ReadEnv("NANO_CLOUD_ADMIN_IS_ADMIN"), "true",
                StringComparison.OrdinalIgnoreCase);

        if (string.IsNullOrWhiteSpace(email))
        {
            stderr.WriteLine("users ensure: --email or NANO_CLOUD_ADMIN_EMAIL is required.");
            return 64;
        }
        if (string.IsNullOrWhiteSpace(displayName))
        {
            stderr.WriteLine("users ensure: --display-name or NANO_CLOUD_ADMIN_DISPLAY_NAME is required.");
            return 64;
        }
        if (string.IsNullOrWhiteSpace(password))
        {
            stderr.WriteLine("users ensure: --password or NANO_CLOUD_ADMIN_PASSWORD is required.");
            return 64;
        }
        ...
        else
        {
            ...
            stdout.WriteLine("              Pass --update-password (or set NANO_CLOUD_ADMIN_UPDATE_PASSWORD=true) to overwrite.");
        }
        ...
        bool isAdmin)
    {
        ...
        if (string.IsNullOrWhiteSpace(email))
        {
            stderr.WriteLine($"users {verb}: --email or NANO_CLOUD_ADMIN_EMAIL is required.");
            return 64;
        }
        ...
        IServiceProvider services, TextWriter stdout, TextWriter stderr)
    {
        ...
        var factory = services.GetService<NanoCloud.Api.Ai.Onnx.IOnnxInferenceSessionFactory>();
        ...
        try
        {
            ...
            using var lease = factory.Acquire(new NanoCloud.Api.Ai.Onnx.OnnxModelSpec(
                NanoCloud.Api.Ai.Onnx.OnnxModel.PhotoImage, modelPath));
        ...
        catch (NanoCloud.Api.Ai.Onnx.OnnxSessionUnavailableException ex)
        {
            stderr.WriteLine(
        ...
        string[] args, IServiceProvider services, TextWriter stdout, TextWriter stderr)
    {
        ...
        var alpr = services.GetRequiredService<IOptions<NanoCloud.Api.Plates.PlatesAlprOptions>>().Value;
        var face = services.GetRequiredService<IOptions<NanoCloud.Api.Plates.PlatesFaceRedactionOptions>>().Value;
    ...
    private static void ValidateAlpr(NanoCloud.Api.Plates.PlatesAlprOptions o, TextWriter stdout)
    {
        ...
        switch (provider)
        {
            case NanoCloud.Api.Plates.PlateAlprProvider.Disabled:
                stdout.WriteLine("  status: disabled");
                break;
            case NanoCloud.Api.Plates.PlateAlprProvider.DeterministicDev:
                stdout.WriteLine("  status: ready (deterministic dev/test — non-semantic, not for production)");
                break;
            case NanoCloud.Api.Plates.PlateAlprProvider.Onnx:
    ...
    private static void ValidateFaceRedaction(
        NanoCloud.Api.Plates.PlatesFaceRedactionOptions o, TextWriter stdout)
    {
        var provider = o.ResolveProvider();
        ...
        switch (provider)
        {
            case NanoCloud.Api.Plates.PlateFaceRedactionProvider.Disabled:
                stdout.WriteLine("  status: disabled");
                break;
            case NanoCloud.Api.Plates.PlateFaceRedactionProvider.DeterministicDev:
                stdout.WriteLine("  status: ready (deterministic dev/test — non-semantic, not for production)");
                break;
            case NanoCloud.Api.Plates.PlateFaceRedactionProvider.ExistingNanoCloudFaceDetector:
                ...
                break;
            case NanoCloud.Api.Plates.PlateFaceRedactionProvider.OnnxDedicatedFaceDetector:
        ...
        if (!o.Enabled)
        {
            stdout.WriteLine($"  note: master switch Enabled=false — blurFaces=true returns {NanoCloud.Api.Plates.Redaction.PlateFaceRedactionUnavailableException.Code}");
        }
    }
    ...
    {
        var pipeline = services.GetRequiredService<NanoCloud.Api.Plates.Alpr.IPlateAnalysisPipeline>();
        ...
        var input = new NanoCloud.Api.Plates.Alpr.PlateImageInput(bytes, width, height);
        double total = 0;
        var lastCount = 0;
        for (var i = 0; i < runs; i++)
        {
            ...
            catch (NanoCloud.Api.Plates.Alpr.PlateAnalysisModelException ex)
            {
    ...
    private static async Task<int> BenchmarkFaceRedactionAsync(
        byte[] bytes, int runs, IServiceProvider services, TextWriter stdout, TextWriter stderr)
    {
        var detector = services.GetRequiredService<NanoCloud.Api.Plates.Redaction.IPlateFaceRedactionDetector>();
        if (!detector.IsAvailable)
        {
            stdout.WriteLine($"FaceRedaction benchmark: unavailable ({NanoCloud.Api.Plates.Redaction.PlateFaceRedactionUnavailableException.Code})");
            return 0;
        }
        ...
        var input = new NanoCloud.Api.Plates.Redaction.PlateRedactionImageInput(bytes, width, height);
        double total = 0;
        var lastFaces = 0;
        for (var i = 0; i < runs; i++)
        {
            ...
            catch (NanoCloud.Api.Plates.Redaction.PlateFaceRedactionUnavailableException)
            {
                stdout.WriteLine($"FaceRedaction benchmark: unavailable ({NanoCloud.Api.Plates.Redaction.PlateFaceRedactionUnavailableException.Code})");
                return 0;
            }
    ...
    internal static void ConfigureCliServices(IServiceCollection services, IConfiguration configuration)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cs))
        {
            ...
            // above this call.
            services.AddNanoCloudJobHandlers();
            ...
            services.Configure<NanoCloud.Api.Ai.Video.VideoSemanticSegmentationOptions>(
            ...
            services.Configure<NanoCloud.Api.Plates.PlatesOptions>(
                configuration.GetSection(NanoCloud.Api.Plates.PlatesOptions.SectionName));
            services.Configure<NanoCloud.Api.Plates.PlatesAlprOptions>(
                configuration.GetSection(NanoCloud.Api.Plates.PlatesAlprOptions.SectionName));
            services.Configure<NanoCloud.Api.Plates.PlatesFaceRedactionOptions>(
                configuration.GetSection(NanoCloud.Api.Plates.PlatesFaceRedactionOptions.SectionName));
            services.AddNanoCloudPlates();
            ...
            services.Configure<NanoCloud.Api.Aesthetics.AestheticsOptions>(
                configuration.GetSection(NanoCloud.Api.Aesthetics.AestheticsOptions.SectionName));
    ...
    internal static Task<int> AiOnnxRuntimeInfoAsync(
        ...
        TextWriter stderr)
    {
        var init = services.GetService<NanoCloud.Api.Ai.Onnx.OnnxDirectRuntimeInitializer>();
        if (init is null)
        {
        ...
        services.GetService<NanoCloud.Api.Ai.Onnx.IOnnxInferenceSessionFactory>()?.EnsureNativeProviderInitialized();
        var info = init.GatherInfo();
        stdout.WriteLine($"commit={Environment.GetEnvironmentVariable("NANOCLOUD_GIT_SHA") ?? "unknown"}");
        ...
        if (NanoCloud.Api.Ai.Onnx.OnnxDirectRuntimeInitializer.LastProvidersError is { } err)
        {
    ...
    private static int EmitEmbedDiagnostics(
        ...
        NanoCloud.Api.Ai.Onnx.IOnnxInferenceSessionFactory factory, TextWriter stdout)
    {
        var reference = all[0];
        double maxDrift = 0;
        var bad = false;
    ...
    private static void WriteHelp(TextWriter stdout)
    {
        ...
        stdout.WriteLine("  dotnet NanoCloud.Api.dll users grant-admin   --email <addr>");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll users revoke-admin  --email <addr>");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll db migrate");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll metadata backfill   [options]");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll metadata recompute-effective-dates");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll media derivatives backfill [options]");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll media derivatives failures");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll media derivatives benchmark [--limit N]");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll storage reconcile          [options]");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll jobs enqueue <job>         [options]");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll jobs list");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll jobs run-once              [--max N]");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll jobs worker                [--poll-interval-seconds N]");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll ai status");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll ai models");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll ai profiles");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll ai diagnostics");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll ai seed");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll ai onnx image <models|seed-profiles|benchmark|embed-test|compare>");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll ai face <models|seed-profiles|detect-test|embed-test|compare|benchmark|sample-pairs>");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll plates models validate     [alpr|face-redaction]");
        stdout.WriteLine("  dotnet NanoCloud.Api.dll plates benchmark <alpr|face-redaction> --image <path> [--runs N]");
        stdout.WriteLine();
        ...
        stdout.WriteLine();
        stdout.WriteLine("  --email <addr>           or NANO_CLOUD_ADMIN_EMAIL");
        stdout.WriteLine("  --display-name <name>    or NANO_CLOUD_ADMIN_DISPLAY_NAME");
        stdout.WriteLine("  --password <secret>      or NANO_CLOUD_ADMIN_PASSWORD");
        stdout.WriteLine("  --update-password        or NANO_CLOUD_ADMIN_UPDATE_PASSWORD=true");
        stdout.WriteLine("  --admin                  or NANO_CLOUD_ADMIN_IS_ADMIN=true");
        stdout.WriteLine();
