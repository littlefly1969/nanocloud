...
builder.Services.AddSingleton<IPasswordHasher<NanoCloud.Api.Domain.PrivateVault>,
    PasswordHasher<NanoCloud.Api.Domain.PrivateVault>>();
// TV Personal Area PINs use the same PBKDF2 hasher (registered unconditionally
// so both the web host and the SQLite test host resolve it).
builder.Services.AddSingleton<IPasswordHasher<NanoCloud.Api.Domain.TvPersonalPin>,
    PasswordHasher<NanoCloud.Api.Domain.TvPersonalPin>>();
...
if (!string.IsNullOrWhiteSpace(connectionString))
{
    ...
    builder.Services.AddScoped<
        NanoCloud.Api.Media.IMediaCollectionQueryService,
    ...
    builder.Services.AddScoped<NanoCloud.Api.Party.IPartyMediaService, NanoCloud.Api.Party.PartyMediaService>();
    builder.Services.AddScoped<NanoCloud.Api.Party.IPartyUploadService, NanoCloud.Api.Party.PartyUploadService>();
    builder.Services.AddScoped<NanoCloud.Api.Party.IPartyModerationService, NanoCloud.Api.Party.PartyModerationService>();
    builder.Services.AddScoped<NanoCloud.Api.Party.IPartyFaceSearchService, NanoCloud.Api.Party.PartyFaceSearchService>();
    ...
    builder.Services.AddScoped<IJobQueueAccessor, NanoCloud.Api.Jobs.VideoHlsJobQueueAccessor>();
    ...
    builder.Services.AddScoped<NanoCloud.Api.Admin.AdminJobCatalogService>();
    ...
    builder.Services.AddNanoCloudPlates();
    ...
    builder.Services.AddNanoCloudAesthetics();
    ...
    builder.Services.AddNanoCloudJobHandlers();
    ...
    builder.Services.AddSingleton<NanoCloud.Api.Aesthetics.AestheticUploadSessionCleanupService>();
    builder.Services.AddSingleton<IHostedService>(sp =>
        sp.GetRequiredService<NanoCloud.Api.Aesthetics.AestheticUploadSessionCleanupService>());
...
builder.Services.Configure<NanoCloud.Api.Plates.PlatesOptions>(
    builder.Configuration.GetSection(NanoCloud.Api.Plates.PlatesOptions.SectionName));
// Plates ALPR pipeline config (separate from Ai:Face*). Disabled by default.
builder.Services.Configure<NanoCloud.Api.Plates.PlatesAlprOptions>(
    builder.Configuration.GetSection(NanoCloud.Api.Plates.PlatesAlprOptions.SectionName));
builder.Services.Configure<NanoCloud.Api.Plates.PlatesFaceRedactionOptions>(
    builder.Configuration.GetSection(NanoCloud.Api.Plates.PlatesFaceRedactionOptions.SectionName));
...
builder.Services.Configure<NanoCloud.Api.Aesthetics.AestheticsOptions>(
    builder.Configuration.GetSection(NanoCloud.Api.Aesthetics.AestheticsOptions.SectionName));
...
builder.Services.Configure<NanoCloud.Api.Ai.Video.VideoVisualEmbeddingOptions>(
    builder.Configuration.GetSection(
...
app.MapGet("/health/ready", (HttpContext httpContext) =>
{
    var preload = httpContext.RequestServices
        .GetService<NanoCloud.Api.Ai.Onnx.Face.IOnnxFacePreloadState>();
    if (preload is null)
    {
