...
public static class GalleryQueryParser
{
    ...
    public static bool TryParseMediaScope(
        string? value, out NanoCloud.Api.MediaLibrary.MediaLibraryScope scope, out string? error)
    {
        scope = NanoCloud.Api.MediaLibrary.MediaLibraryScope.Active;
        error = null;
        if (string.IsNullOrWhiteSpace(value)) return true;
        ...
        switch (value.Trim().ToLowerInvariant())
        {
            case "active":
                ...
                return true;
            case "excluded":
                scope = NanoCloud.Api.MediaLibrary.MediaLibraryScope.Excluded;
                return true;
            default:
