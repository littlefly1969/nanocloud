...
using NanoCloud.Api.Party;
using NanoCloud.Api.Tests.Endpoints;
using NanoCloud.Api.Tests.Metadata;
...
namespace NanoCloud.Api.Tests.Party;
...
public sealed class PartyFaceSearchTests
{
    ...
    public async Task Face_Search_No_Face_Is_Reported_Safely()
    {
        ...
        using (var setup = f.Services.CreateScope())
        {
            ...
            var album = new NanoCloud.Api.Domain.Album
            {
                Id = Guid.NewGuid(),
                OwnerUserId = ownerId,
                Name = "Party",
        ...
        var service = new PartyFaceSearchService(
            ...
            sp.GetRequiredService<NanoCloud.Api.Ai.Faces.IFaceSettingsProvider>(),
            sp.GetRequiredService<IAiVectorSerializer>(),
            sp.GetRequiredService<NanoCloud.Api.Storage.IBlobService>(),
    ...
    public async Task Face_Search_Response_Exposes_No_Internals()
    {
        ...
        foreach (var needle in new[]
        {
            ...
            "PayloadJson", "/storage/objects/", "at NanoCloud.",
        })
        {
    ...
    private static async Task RunJobAsync(SqliteWebApplicationFactory f, string jobType)
    {
        using (var scope = f.Services.CreateScope())
        {
            await scope.ServiceProvider.GetRequiredService<IJobQueue>()
                .EnqueueAsync(jobType, new NanoCloud.Api.Ai.Jobs.AiBackfillJobPayload());
        }
        for (var i = 0; i < 50; i++)
        {
            using var scope = f.Services.CreateScope();
