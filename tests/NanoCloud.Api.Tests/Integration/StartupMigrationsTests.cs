...
namespace NanoCloud.Api.Tests.Integration;
    ...
    public async Task InitializeAsync()
    {
        try
        {
            _container = new PostgreSqlBuilder()
                ...
                .WithDatabase("nanocloud_startup")
                .WithUsername("nanocloud")
                .WithPassword("nanocloud")
                .Build();
...
internal sealed class MigrateOnStartupFactory : WebApplicationFactory<Program>
{
    ...
    public MigrateOnStartupFactory(string connectionString, bool migrateOnStartup)
    {
        ...
        _storageRoot = Path.Combine(Path.GetTempPath(), $"nanocloud-startup-mig-{Guid.NewGuid():N}");
