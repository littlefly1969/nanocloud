...
namespace NanoCloud.Api.Tests.Storage;
...
public sealed class DerivedStorageTests : IDisposable
{
    ...
    {
        _derivedRoot = Path.Combine(Path.GetTempPath(), $"nanocloud-derived-{Guid.NewGuid():N}");
    ...
    public async Task Preview_Endpoint_Persists_Medium_Derivative_In_Derived_Root()
    {
        ...
        using var scope = _factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<NanoCloud.Api.Data.AppDbContext>();
        var medium = await (
